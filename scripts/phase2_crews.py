...
def _build_phase2_markdown(payload: dict[str, Any]) -> str:
    ...
    for division in payload.get("divisions", []):
        if not isinstance(division, dict):
            continue
        division_name = str(division.get("division", "division")).title()
        lines.append("")
        lines.append(f"## {division_name} Division")
...
def run_phase2_divisions(config: dict[str, Any], division: str = "all", force: bool = False) -> dict[str, Any]:
    ...
    if division not in {"all", "trading", "websites", "content_studio"}:
        return {
            "ok": False,
            "error": "invalid_division",
            "message": "division must be one of: all, trading, websites, content_studio",
        }
    ...
    for division_id in divisions_to_run:
        try:
            if division_id == "content_studio":
                from content_studio import run_content_studio  # pylint: disable=import-outside-toplevel

                content_studio_result = run_content_studio(config=config)
                result = {
                    "division": "content_studio",
                    "ok": bool(content_studio_result.get("ok", True)),
                    "engine": "content_studio_orchestrator",
                    "status": str(content_studio_result.get("status", "GREEN")).upper(),
                    "spec_file": str(ROOT / "crews" / "content_studio.yaml"),
                    "final_output": str(content_studio_result.get("notes", "")),
                    "task_outputs": [],
                    "warnings": [],
                    **content_studio_result,
                }
            else:
